    ...
    execute: async function (api, message, args, config, utils) {
        ...
        if (response.knowledge_panel.title !== 'N/A' && response.knowledge_panel.description !== 'N/A') {
            ...
            if (response.knowledge_panel.born)
                x.push('Born: ' + response.knowledge_panel.born);
            if (response.knowledge_panel.died)
                x.push('Died: ' + response.knowledge_panel.died);
            if (response.knowledge_panel.spouse)
                x.push(`Spouse: ${response.knowledge_panel.spouse}`);
            if (response.knowledge_panel.children)
                x.push(`Children: ${response.knowledge_panel.children}`);
            if (response.knowledge_panel.parents)
                x.push(`Parents: ${response.knowledge_panel.parents}`);
            return api.sendMessage(x.join('\n'), message.threadID);
        }
        else {
            ...
            if (x.join('\n').length > 200) {
                api.sendMessage('Too avoid huge block of text annoying everyone...\n' +
                    'The results were sent to your DMs.\n' +
                    'Kindly check them out.', message.threadID);
