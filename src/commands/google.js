...
module.exports = {
	...
	async execute(api, message, args) {
		...
		if (
			...
			response.knowledge_panel.description !== 'N/A'
		) {
			...
			if (response.knowledge_panel.children)
				data.push(`Children: ${response.knowledge_panel.children}`);
			if (response.knowledge_panel.parents)
				data.push(`Parents: ${response.knowledge_panel.parents}`);
			...
			if (data.join('\n').length > 500) {
				api.sendMessage(
					'To avoid a massive block of message annoying everyone the results was sent to your DMs.',
					message.threadID
				);
