...
module.exports = {
	...
	execute(api: { sendMessage: (arg0: string, arg1: any) => void }, message: { threadID: any }, args: any[]) {
		...
		if (!log[thread_id]) return api.sendMessage('No messages in this thread.', thread_id);
		if (!args[0]) {
		...
		if (!log[thread_id][iD]) return api.sendMessage('No message with that ID.', thread_id);
