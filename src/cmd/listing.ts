	...
	execute: function (
		...
		switch (args[0]) {
			...
			case 'end':
				...
				if (cache[threadID].details) txt.push(cache[threadID].details);
				if (cache[threadID].limit) txt.push(`Limit: ${cache[threadID].limit}`);
				txt.push(`Total: ${cache[threadID].list.length}`);
				txt.push(`List:`);
				cache[threadID].list.forEach((item: any) => {
					txt.push(`\t${item}`);
				});
			...
			case 'stat':
				...
				txt.push(`Total: ${cache[threadID].list.length}`);
				txt.push(`List:`);
				cache[threadID].list.forEach((item: any, index: number) => {
